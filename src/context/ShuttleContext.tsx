...
import type { BusId, Direction, Roster, Kid } from '../types/shuttle';
import { UseLocation, LocationType } from '../components/UseLocation';
...
export type ShuttleState = {
  ...
  setSending: (s: boolean | ((prev: boolean) => boolean)) => void;
...
export const ShuttleProvider: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
  ...
  // Background Location Logic moved here
  const { location, errorMsg } = UseLocation(sending);

  // Log error but don't crash UI
  if (errorMsg) {
    console.warn('Location Error in Context:', errorMsg);
  }
  ...
  const value = useMemo(() => ({
    date, bus, direction, rosterMap, sending, location,
    setDate, setBus, setDirection, setSending, updateKidStatus
  }), [date, bus, direction, rosterMap, sending, location]);
